...
--   3. Loop spam teleport đến đảo trung gian
--   4. Delay nhỏ nếu cần (phụ thuộc khoảng cách)
...
local function doIntermediateTeleport(targetCF, speed)
    ...
    task.spawn(function()
        ...
        -- Bước 2: Xác định thời gian spam (xa hơn → spam lâu hơn)
        ...
        print("🚀 Tele trung gian → " .. island.name .. " (dist=" .. math.floor(dist) .. ")")

        -- Bước 3: Loop spam teleport đến đảo trung gian
        ...
        -- Bước 4: Delay nhỏ sau khi đến đảo trung gian (nếu cần server load)
...
getgenv().FlySpeed = 180            -- Tốc độ bay (càng cao bay càng nhanh, nhưng dễ bị phát hiện)
...
getgenv().MaxBringCount = 2         -- Max mob được bring mỗi lần (set qua dropdown Settings)
...
getgenv().AuraFruit = false         -- Aura Fruit mode: đứng sát mob (Y+10), kích hoạt cả Topi + aura attack
...
Tabs.Settings:CreateDropdown("SpeedTween", {
    Title = "Speed Tween",
...
--------------------------------------------------------------------
-- ATTACK SOUND / ÂM THANH ATTACK — CORE SCRIPT
-- Toàn bộ logic topi attack script, kiểm soát bằng AttackSoundEnabled
--------------------------------------------------------------------
getgenv().AttackSoundEnabled = false
getgenv().AttackSoundToggleOn = false
...
do
    ...
    -- Heartbeat connection lưu làm upvalue để stop function có thể disconnect
    local _atkSoundHBConn = nil

    local function BuCuAnhKo_StartLoops()
        -- Loop 1: FastCore — chỉ chạy khi AttackSoundEnabled = true
        task.spawn(function()
            while task.wait(DanhNhanhBuCu36.Rate) do
                if not getgenv().AttackSoundEnabled then break end
                pcall(DanhNhanhBuCu36.FastCore)
            end
        end)
        -- Loop 2: Heartbeat — tự disconnect khi tắt
        if _atkSoundHBConn then pcall(function() _atkSoundHBConn:Disconnect() end) end
        _atkSoundHBConn = X003.Heartbeat:Connect(function()
            if not getgenv().AttackSoundEnabled then
                if _atkSoundHBConn then _atkSoundHBConn:Disconnect(); _atkSoundHBConn = nil end
                return
            end
    ...
    function BuCuAnhKo.Start()
        pcall(BuCuAnhKo_DisableCam)
        BuCuAnhKo_StartLoops()
    end

    -- Junk/padding code (giữ nguyên)
    local Z001 = {}
    local Z002 = {}
    local Z003 = {}
    local Z004 = {}
    ...
    -- Night Slayer Hub extended attack loop — kiểm tra AttackSoundEnabled
    local _nsHBRunning = false
    local function startNSHubLoop()
        if _nsHBRunning then return end
        _nsHBRunning = true
        local RS2 = game.ReplicatedStorage
        local N2 = require(RS2.Modules.Net)
        local C2 = require(RS2.Modules.CombatUtil)
                ...
                pcall(function()
                    ...
                    for _, m in ipairs(workspace.Enemies:GetChildren()) do
                        ...
                        if h and u and u.Health > 0 and (h.Position - r.Position).Magnitude <= 60 then
                            ...
                            C2:ApplyDamageHighlight(m, c, w, h)
                        end
                    end
                end)
            end
    ...
    -- Expose start/stop ra ngoài scope thông qua getgenv()
    getgenv()._startAttackSoundFn = function()
        BuCuAnhKo.Start()
        pcall(startNSHubLoop)
    end

    getgenv()._stopAttackSoundFn = function()
        getgenv().AttackSoundEnabled = false
        if _atkSoundHBConn then
            pcall(function() _atkSoundHBConn:Disconnect() end)
            _atkSoundHBConn = nil
        end
        _nsHBRunning = false
    end
...
--------------------------------------------------------------------
-- TOGGLE: ATTACK SOUND (ÂM THANH ATTACK)
-- Đặt phía trên Auto Attack, hoạt động độc lập
--------------------------------------------------------------------
Tabs.Settings:CreateToggle("AttackSound", {
    Title = "Attack Sound",
    Default = false,
    Callback = function(v)
        getgenv().AttackSoundToggleOn = v
        if v then
            getgenv().AttackSoundEnabled = true
            getgenv()._startAttackSoundFn()
        else
            -- Chỉ dừng Koby nếu AutoAttack cũng đang tắt
            if not getgenv().FastAttack then
                getgenv()._stopAttackSoundFn()
            else
                -- AutoAttack đang bật → chỉ tắt toggle, Koby vẫn chạy
                getgenv().AttackSoundEnabled = true
            end
        end
    end
...
--------------------------------------------------------------------
-- TOGGLE: AUTO ATTACK (ĐÃ FIX — bật/tắt Koby attack đúng cách)
--------------------------------------------------------------------
Tabs.Settings:CreateToggle("Initialize", {
    ...
    Callback = function(v)
        ...
        if v then
            -- Bật → kích hoạt Koby attack
            getgenv().AttackSoundEnabled = true
            getgenv()._startAttackSoundFn()
        else
            -- Tắt → chỉ dừng Koby nếu Attack Sound toggle cũng đang tắt
            if not getgenv().AttackSoundToggleOn then
                getgenv()._stopAttackSoundFn()
            else
                -- Attack Sound vẫn bật → giữ Koby chạy
                getgenv().AttackSoundEnabled = true
            end
        end
    end
...
Tabs.Settings:CreateToggle("AuraFruit", {
    Title = "Aura Fruit",
    Default = false,
    Callback = function(v)
        getgenv().AuraFruit = v
        -- Koby script sẽ được load tự động qua monitor loop khi AuraFruit = true
    end
})
...
Tabs.Settings:CreateDropdown("MaxBringCount", {
    Title = "Max Bring Mob",
    Description = "Số mob tối đa bring mỗi lần (không tính mob đang farm)",
    Values = {"2", "3", "4", "5", "6", "7", "8"},
    Default = 1,
    Callback = GuardDropdown(function(v)
        getgenv().MaxBringCount = tonumber(v) or 2
    end),
})
...
-- - Y-snap khi chênh lệch > YTeleportThreshold
-- - Fly + teleport đồng thời khi dist <= 200
-- ============================================================

local YTeleportThreshold = 150
...
local function _tp(targetCF, speed)
    ...
    local yDiff = math.abs(targetCF.Position.Y - hrp.Position.Y)
    if yDiff > YTeleportThreshold then
        if currentTween then pcall(function() currentTween:Cancel() end); currentTween = nil end
        currentTweenSpeed  = 0
        currentTweenTarget = nil
        hrp.CFrame = CFrame.new(hrp.Position.X, targetCF.Position.Y, hrp.Position.Z)
        -- Không delay: tiếp tục tween ngay sau snap
        if not getgenv().IsFarming then return end
        dist = (targetCF.Position - hrp.Position).Magnitude
    end

    if dist < 1 then return end  -- Đã đến nơi, không cần làm gì

    -- ── Gần đích (< 350 studs): teleport thẳng tới đích, chạy song song với tween ──
    -- Không còn tăng speed như trước; teleport "kéo" nhân vật tới đích ngay lập tức,
    -- tween bên dưới vẫn được tạo/giữ song song như bình thường.
    if dist < 350 then
        hrp.CFrame = targetCF
        dist = 0
    end
...
local function CalculateHeight(mobPosition)
    if getgenv().AuraFruit and _G.ChooseWP == "Blox Fruit" then
        return mobPosition.Y + 15  -- Trang bị Fruit + bật Aura Fruit: Y+15
    end
    if _G.ChooseWP == "Blox Fruit" then
        return mobPosition.Y + 15  -- Fruit: Y+15
    end
    if _G.ChooseWP == "Melee" or _G.ChooseWP == "Sword" then
        return mobPosition.Y + 25  -- Melee/Sword: Y+25
    end
...
-- Up:        Y+25 (Melee/Sword) / Y+30 (Gun) / Y+15 (Fruit/AuraFruit), đứng thẳng trên mob
-- AuraFruit / Blox Fruit: luôn Y+15
--------------------------------------------------------------------
local function GetFarmCFrame(mob)
    ...
    -- Offset vị trí farm so với mob: X+4 / Z+0, Y theo loại vũ khí bên dưới
    local OFFSET_X = 4
    local OFFSET_Z = 0

    -- Trang bị Fruit + bật Aura Fruit: đứng sát mob X/Z, Y+15
    if getgenv().AuraFruit and _G.ChooseWP == "Blox Fruit" then
        targetPos = Vector3.new(mobPos.X + OFFSET_X, mobPos.Y + 15, mobPos.Z + OFFSET_Z)
    elseif _G.ChooseWP == "Blox Fruit" then
        targetPos = Vector3.new(mobPos.X + OFFSET_X, mobPos.Y + 15, mobPos.Z + OFFSET_Z)
    else
        -- Up (mặc định)
        if _G.ChooseWP == "Melee" or _G.ChooseWP == "Sword" then
            targetPos = Vector3.new(mobPos.X + OFFSET_X, mobPos.Y + 25, mobPos.Z + OFFSET_Z)
        else
            targetPos = Vector3.new(mobPos.X + OFFSET_X, mobPos.Y + 30, mobPos.Z + OFFSET_Z)
        end
    end
...
--------------------------------------------------------------------
--------------------------------------------------------------------
-- BRING MOBS - Logic Bring Mob Mới
-- Tính năng:
--   - Bring mob cùng loại về vị trí mob farm (thay vì vị trí trung bình)
--   - Phạm vi bring: 350 studs quanh mob farm
--   - Check TargetPlayer: nếu có player khác 500 studs quanh localplayer → không bring
--   - Teleport + Tween dựa theo FlySpeed ÷ 1.5
--   - Anti-lag: SimulationRadius gọi 1 lần, chỉ destroy Animator khi cần
-- MaxBring: giới hạn số mob được bring qua getgenv().MaxBringCount
--------------------------------------------------------------------
task.spawn(function()
    while task.wait(0.5) do
        pcall(function()
            ...
            -- Lấy mob đang farm làm mốc bring
            ...
            --// CHECK TARGETPLAYER: 500 studs quanh localplayer
            local myHRP = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
            if not myHRP then return end
            local myPos = myHRP.Position
            
            -- Kiểm tra có TargetPlayer (player khác) trong 500 studs không
            local hasTargetPlayer = false
            for _, p in ipairs(Players:GetPlayers()) do
                if p ~= player and p.Character then
                    local pHRP = p.Character:FindFirstChild("HumanoidRootPart")
                    if pHRP and (pHRP.Position - myPos).Magnitude <= 500 then
            ...
            -- Nếu có TargetPlayer thì không bring
            if hasTargetPlayer then return end

            -- Vị trí cố định của mob farm - tất cả mob bring về đây
            local farmPos    = targetHRP.Position
            local targetName = targetMob.Name
            local maxBring   = getgenv().MaxBringCount or 2

            -- Tính toán khoảng cách teleport + tween dựa theo FlySpeed
            -- Formula: khoảng cách = FlySpeed ÷ 1.5
            local flySpeed = getgenv().FlySpeed or 180
            local teleportDistance = flySpeed / 1.5

            -- SimulationRadius gọi 1 lần, không lặp trong loop mob
            pcall(function()
                sethiddenproperty(player, "SimulationRadius", math.huge)
            end)

            -- Bring mob cùng loại trong phạm vi 350 quanh mob farm
            local brought = 0
            for _, v in ipairs(workspace.Enemies:GetChildren()) do
                if brought >= maxBring then break end
                if v ~= targetMob and v.Name == targetName then
                    local hum = v:FindFirstChild("Humanoid")
                    local hrp = v:FindFirstChild("HumanoidRootPart")
                    if hum and hrp and hum.Health > 0 then
                        local dist = (hrp.Position - farmPos).Magnitude
                        
                        -- Kiểm tra mob nằm trong phạm vi 350 studs quanh mob farm
                        if dist > 8 and dist <= 350 then
                            task.spawn(function()
                                pcall(function()
                                    -- Nếu khoảng cách > teleportDistance: teleport trước
                                    if dist > teleportDistance then
                                        hrp.CFrame = CFrame.new(farmPos + Vector3.new(0, 3, 0))
                                        task.wait(0.1)
                                    end
                                    
                                    -- Tween từ vị trí hiện tại (hoặc sau teleport) về farmPos
                                    ...
                                    local tween = TweenService:Create(
                                        hrp,
                                        tweenInfo,
                                        { CFrame = CFrame.new(farmPos + Vector3.new(0, 0, 0)) }
                                    )
                                    
                                    -- Vô hiệu hóa tính năng của mob
                                    hrp.CanCollide = false
                                    hrp.Velocity = Vector3.zero
                                    hum.WalkSpeed = 0
                                    hum.JumpPower = 0
                                    
                                    -- Destroy Animator nếu tồn tại
...
--
-- Luồng:
--   1. Fly đến vị trí NPC, đợi đến nơi (timeout 30s)
--   2. Đứng yên 2s
--   3. Kích hoạt TravelToSubmergedIsland
--   4. Đứng yên 2s
--   5. Return → farm/travel tiếp tục bình thường
--------------------------------------------------------------------
...
function startLevelFarm()
    ...
    local _spawnArrivedAt = 0    -- tick() khi đến điểm spawn (để delay 2s trước khi đổi điểm)
    ...
    levelFarmConn = RunService.Heartbeat:Connect(function(dt)
        ...
        pcall(function()
            ...
            if enemy then
                ...
                -- Không có mob → fly lần lượt qua các EnemySpawn thực tế của mob đang farm
                getgenv().CurrentTargetMob = nil

                -- Rebuild danh sách spawn khi đổi mob (lên lv đổi khu vực)
                if _spawnLastMon ~= activeMon then
                    ...
                    if spawnsFolder then
                        for _, sp in ipairs(spawnsFolder:GetChildren()) do
                            if sp.Name == activeMon or string.find(sp.Name, activeMon, 1, true) then
                                local part = sp:IsA("BasePart") and sp
                                    or sp:FindFirstChildWhichIsA("BasePart")
                                if part then
                                    -- Y + 30 để bay phía trên điểm spawn
                    ...
                    print("🗺️ EnemySpawns tìm được: " .. #_spawnPoints .. " điểm cho " .. tostring(activeMon))
                end

                -- Tween đến điểm spawn hiện tại
                -- Đến nơi → delay 2s, sau đó mới chuyển sang điểm tiếp theo
                if _spawnPoints and #_spawnPoints > 0 then
                    ...
                    if root and (root.Position - targetSP.Position).Magnitude <= 20 then
                        -- Đến điểm spawn → bắt đầu đếm delay 2s
                        if _spawnArrivedAt == 0 then
                            _spawnArrivedAt = tick()
                        elseif tick() - _spawnArrivedAt >= 0.1 then
                            -- Hết 2s, vẫn chưa có mob (nếu có mob thì branch if enemy đã bắt) → sang điểm tiếp
                            _spawnArrivedAt = 0
                            _spawnIndex     = (_spawnIndex % #_spawnPoints) + 1
                        end
                    else
                        _spawnArrivedAt = 0  -- chưa đến → reset timer
                    end
                end
            end
        end)
        ...
        if bigMirror.Other.Transparency == 0 or workspace.Enemies:FindFirstChild("Cake Prince") then
            ...
            if cakePrince then
                ...
                if cpHRP then
                    ...
                    local yOff
                    if getgenv().AuraFruit or _G.ChooseWP == "Blox Fruit" then
                        yOff = 15
                    else
                        yOff = 25
                    end
...
-- Tìm mob "Destroy" xuất hiện trong dungeon (ưu tiên cao nhất)
-- Tìm cả trong workspace.Enemies và trong zone folder hiện tại
local function FindDestroy()
    -- Tìm trong workspace.Enemies trước
    for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
        if isDestroyTarget(enemy) then
            return enemy
        end
    end

    -- Fallback: tìm trong toàn bộ workspace (phòng khi Destroy spawn ngoài Enemies)
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj.Name ~= "Enemies" and isDestroyTarget(obj) then
            return obj
        end
    end
...
--// ================= FAST ATTACK (KOBY) =================
-- Thay toàn bộ logic attack cũ bằng koby script.
-- Script được load MỘT LẦN DUY NHẤT khi farm bật, FastAttack toggle bật, hoặc AuraFruit bật.
-- Monitor loop tự động phát hiện và load khi điều kiện thỏa mãn.
local _kobyScriptLoaded = false
local function ActivateKobyAttack()
    if _kobyScriptLoaded then return end
    _kobyScriptLoaded = true
    pcall(function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/TopiHub1909/TopiHub/refs/heads/main/Fast_AttackV1"))()
    end)
    print("✅ Koby attack script loaded!")
end

-- Monitor: tự load koby khi bất kỳ điều kiện attack nào được kích hoạt
spawn(function()
    while task.wait(0.5) do
        if getgenv().FastAttack or getgenv().IsFarming or getgenv().AutoMaterial or getgenv().AuraFruit then
            ActivateKobyAttack()
        end
    end
end)
...
--// ================= AURA FRUIT ATTACK =================
--// FAST ATTACK ALL (FAKE BLOX FRUIT) - NO EQUIP FRUIT
--// Chỉ chạy khi getgenv().AuraFruit = true VÀ đang farm.
    ...
    local function Scan(folder)
        for _, v in pairs(folder:GetChildren()) do
            if v ~= char and v:FindFirstChild("HumanoidRootPart") and _AF_IsAlive(v) then
                ...
                if dist <= _AF_CONFIG.RANGE then table.insert(list, v) end
            end
        end
    end
...
local function _AF_AttackFruit(targets)
...
-- Heartbeat loop: chỉ chạy khi AuraFruit = true và đang farm
RunService.Heartbeat:Connect(function()
    if not getgenv().AuraFruit then return end
    if not (getgenv().IsFarming or getgenv().FastAttack or getgenv().AutoMaterial) then return end
    if getgenv().BuddhaTransforming then return end
    local now = tick()
    if (now - _AF_lastAttack) < _AF_CONFIG.ATTACK_INTERVAL then return end
    _AF_lastAttack = now
    local targets = _AF_GetTargets()
    if #targets == 0 then return end
    _AF_AttackFruit(targets)
end)
